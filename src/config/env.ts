...
export const env = {
  ...
  // Default to the App Router mock API. .env.local is gitignored, so a missing
  // production value would otherwise send cafe requests to `/cafes` (the HTML
  // page) instead of `/api/v1/cafes`.
  apiBaseUrl: trimEnv(process.env.NEXT_PUBLIC_API_BASE_URL) || DEFAULT_API_BASE_URL,
